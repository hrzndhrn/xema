defmodule Xema.BooleanTest do
  ...
  describe "'boolean' schema" do
    ...
    test("is_valid?/2 with value true", %{schema: schema}, do: assert(is_valid?(schema, true)))

    test("is_valid?/2 with value false", %{schema: schema}, do: assert(is_valid?(schema, false)))
    ...
    test(
      "validate/2 with value true",
      %{schema: schema},
      do: assert(validate(schema, true) == :ok)
    )

    test(
      "validate/2 with value false",
      %{schema: schema},
