defmodule JsonSchemaTestSuite.Draft6.TypeTest do
  use ExUnit.Case
  ...
  describe ~s|integer type matches integers| do
    ...
    test ~s|a float with zero fractional part is an integer|, %{schema: schema} do
  ...
  describe ~s|number type matches numbers| do
    ...
    test ~s|a float with zero fractional part is a number (and an integer)|, %{schema: schema} do
      assert valid?(schema, 1.0)
    end
